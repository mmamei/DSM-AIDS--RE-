...
* ROC and PR curves, AUC.
* Describe regression metrics: RMSE, MSE, MAD.
* Model selection: training, testing, validation set.
...
* Describe pre-pruning and post-pruning mechanisms.
...
* Batch, stochastic, and minibatch training.
* Solve an exercise / invent a small dataset and apply a linear model by hand.
...
* Describe the main activation functions and explain the vanishing gradient problem.
* What are the hyperparameters of a neural network? How can they be set? In particular, describe how to set the number of epochs based on training and validation error.
...
* What metrics are used to evaluate clustering algorithms? Internal vs. external.
* How does an agglomerative clustering algorithm work? How is the distance between clusters defined, for example single linkage?
